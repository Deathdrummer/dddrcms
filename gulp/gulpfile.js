const 	domain			= 'balito.loc', // прописать домен
...
async function build() {
	...
	publicDirs.forEach(function(dir) {
		...
		src('../public/js/'+dirName+'.js').pipe(uglify().on('error', function(err) {
...
function buld_from_repository(done) {
	...
	// components css
	src(repoDir+'components/**/*.{sass,scss}').on('end', done)
	...
	// assets css
	src(repoDir+'css/assets/assets.{sass,scss}').on('end', done)
	.pipe(sass({includePaths: bourbon}).on("error", notify.onError()))
...
function common_functions_from_repository() {
	return src([repoDir+'js/common.js', repoDir+'js/functions.js'])
	//.pipe(babel({presets: ['@babel/preset-env'], compact: false}))
	.pipe(uglify().on('error', function(err) {
		gulpUtil.log(gulpUtil.colors.red('[Error]'), err.toString());
		this.emit('end');
...
function plugins_js_from_repository() {
	return src(repoDir+'js/plugins/**/*.js')
	.pipe(order([
		'jquery/jquery-3.2.1.min.js',
		'jquery/jquery.migrate.js',
		'jquery/ui/**/*.js',
...
function assets_css_from_repository(done) {
	return src(repoDir+'css/assets/assets.{sass,scss}').on('end', done)
	.pipe(sass({includePaths: bourbon}).on("error", notify.onError()))
	.pipe(rename({suffix: '.min', prefix : ''}))
	.pipe(autoprefixer(['> 1%', 'last 2 versions', 'Firefox >= 20', 'iOS 7', 'ie 9']))
	.pipe(cleanCSS())
